...
- **振り返り**: 終了時に「今日の成果（コメント）」と「集中度（5段階）」を入力。
- **ルール**: 1回あたりの上限を90分に設定。
...
### 🏆 ゲーミフィケーション・演出
- **メダルシステム**: 累計獲得EXPに応じてランク（ブロンズ〜マスター）が変動し、ステータス画面にメダル画像を表示。
- **週間ランキング**: 直近7日間の獲得EXPを集計し、上位3名＋自分の順位をステータス画面に表示。
- **リボン・称号**: 特定の条件（例：継続学習）などで付与されるリボンを表示（基盤実装済み）。
- **ランクアップ通知**: 勉強承認時にランクが上がった場合、自動でお祝いメッセージと新しいメダル画像を送信。
- **ガチャ制限**: 「Rank D」以上にならないとガチャが引けないロック機能。
- **週間グラフ**: ステータス詳細画面で、直近7日間の学習時間を棒グラフで可視化。

### 👶 オンボーディング
- **初期登録フロー**: 初回メッセージ送信時に名前入力を促し、登録完了と同時に500EXP（ウェルカムボーナス）を付与。
...
### 🎨 演出・UX強化案 (Visual Enhancements)
1.  **アチーブメント（実績）バッジの実装**
    - 現在は基盤のみのリボン機能を拡張し、「早起き（朝8時前学習）」「継続（3日連続）」などの条件判定ロジックを実装して自動付与する。
2.  **季節限定テーマ**
    - ハロウィンやクリスマスなど、季節に合わせてメダルや背景のデザインを切り替えるイベント機能。
3.  **学習ストリークボーナス**
    - 毎日勉強を続けることで、獲得EXPにボーナス倍率（x1.1, x1.2...）がかかる仕組み。
4.  **親向け通知設定**
    - 通知が多すぎる場合の対策として、通知レベル（全て通知 / 重要のみ / オフ）を設定できるようにする。
