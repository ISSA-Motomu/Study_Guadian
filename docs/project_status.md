...
- **詳細レポート**: 「詳細ステータス」にて、週間・月間の学習時間を教科別積み上げグラフで表示。
- **偏差値判定**: 佐賀県統計モデルに基づき、日々の学習量から偏差値と合格圏内の高校（佐賀西、致遠館など）を判定・表示。
...
### 🎁 デイリーボーナス
- **初回学習ボーナス**: その日最初の勉強（5分以上）を記録すると、自動的に **+30 EXP** を付与。
- **目的**: 「とりあえず机に向かう」習慣作りを支援。
...
- **中断・再開・取消**: 勉強中の一時中断や、間違えて開始した場合のキャンセル機能。
...
- **アカウント復旧**: `!admin` コマンドによる管理者権限の復旧・切り替え機能。
...
### 💡 新機能アイデア (Proposed)
1.  **目標設定機能 (Goal Setting)**
    - 「今週は合計5時間勉強する」「今月はランクCを目指す」などの目標を設定し、達成時にボーナスを付与。
2.  **親からの「クエスト」発注**
    - 通常のジョブとは別に、「テストで80点以上取る」「本を1冊読む」などの特別クエストを親が発行できる機能。
3.  **学習リマインダー**
    - 設定した時間（例: 17:00）になっても勉強記録がない場合、Botから「そろそろ勉強の時間だよ！」と通知を送る。
