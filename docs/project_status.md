...
1.  **レベルアップ通知**
    - ランクアップ時に、ステータス画面を開かなくても自動でお祝いメッセージと画像を送る。
2.  **アチーブメント（実績）バッジ**
    - 「算数マスター（算数を10回勉強）」「早起き鳥（朝8時前に勉強）」などの特定条件でバッジを付与し、コレクション要素を追加。
3.  **グラフのインライン表示**
    - Looker Studioに飛ばなくても、LINE上で簡易的な週間グラフ（画像）を生成して表示する（QuickChart.ioなどの利用）。
4.  **季節限定テーマ**
